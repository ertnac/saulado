...
function App() {
  ...
  const getFilteredPool = () => {
    const all = getAllCardsRecursively(activeDeck);
    if (studyFilter === "new")
      return all.filter((c) => !c.attempt_count || c.attempt_count === 0);
    return all;
  };
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden w-full">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b flex items-center justify-between px-4 md:px-10 shrink-0 z-20 gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={prepareStudy}
              className="bg-slate-900 text-white p-2.5 md:px-8 rounded-xl text-[10px] md:text-xs font-black shadow-xl uppercase whitespace-nowrap transition-all hover:scale-105 active:scale-95"
            >
            ...
            <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 pb-24">
              ...
              <section className="bg-white p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={saveCard}
                    className="w-full md:flex-1 bg-indigo-600 text-white px-5 py-3 rounded-xl font-black text-xs shadow-lg uppercase transition-all hover:bg-indigo-700"
                  >
                    Save Card
                  </button>
                </div>
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-3 px-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="text-[9px] md:text-[10px] bg-slate-50 text-slate-400 px-3 py-1.5 rounded-full font-bold hover:bg-slate-200 uppercase"
                    >
                      Shuffle
                    </button>
                    <button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
          ...
          >
            {isModalOpen === "settings" && (
              <>
                ...
                <p className="text-[10px] text-slate-400 uppercase font-black mb-2 text-center">
                  Ilang cards sasagutan? ({getFilteredPool().length} available)
                </p>
                <input
                  type="number"
                  value={cardLimit}
                  onChange={(e) =>
                    setCardLimit(
                      Math.min(e.target.value, getFilteredPool().length),
                    )
                  }
                ...
                <button
                  ...
                  className="bg-indigo-600 text-white py-4 rounded-xl font-black shadow-lg uppercase tracking-widest text-xs transition-all hover:bg-indigo-700"
                >
                  Start Study
                </button>
              </>
            ...
            {isModalOpen === "magic" && (
              <>
                <h3 className="font-black text-xl mb-4 italic leading-none">
                  Magic Import ✨
                </h3>
                <input
                  type="file"
